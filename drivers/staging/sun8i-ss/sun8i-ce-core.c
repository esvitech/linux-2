...
/*	.alg_hash = { CE_ID_NOTSUPP, SS_OP_MD5, SS_OP_SHA1, SS_OP_SHA224,
	...
	.alg_hash = { CE_ID_NOTSUPP, CE_ID_NOTSUPP, SS_OP_SHA1, CE_ID_NOTSUPP,
		CE_ID_NOTSUPP, CE_ID_NOTSUPP, CE_ID_NOTSUPP, },
...
int sun8i_ss_run_task(struct sun8i_ss_ctx *ss, int flow, const char *name)
{
	...
	dev_info(ss->dev, "Method %x mode %x\n",
	...
	if (ss->chanlist[flow].method == SS_OP_PRNG) {
		/* grab continue mode */
		v |= BIT(18);
		/* TODO */
		cet->t_src[0].len = 5;
	...
	for (i = 0; i < MAX_SG; i++) {
		/*if (!cet->t_src[i].addr && ss->chanlist[flow].op_mode != )
			break;*/
		if (!cet->t_dst[i].addr)
			break;
		dev_info(ss->dev, "Processing SG %d %s %x %d to %d\n", i, name, v,
	...
	if (ss->chanlist[flow].next_iv) {
		...
		for (i = 0; i < 4; i++) {
			if (flow)
				*iv = readl(ss->base + 0x48 + i * 4);
			else
				*iv = readl(ss->base + 0x34 + i * 4);
			iv++;
		}
	}
theend:
...
*/
	...
	for (i = 0; i < ARRAY_SIZE(ss_algs); i++) {
		...
		case CRYPTO_ALG_TYPE_AHASH:
			ce_method = ss->variant->alg_hash[ss_algs[i].ce_algo_id];
			if (ce_method == CE_ID_NOTSUPP)
				break;
			ss_algs[i].mode = ce_method;
	...
	#ifdef CRYPTO_DEV_SUN8I_SS_PRNG
