...
{
	sch->seed = kmalloc(sch->seedsize, GFP_DMA | GFP_ATOMIC);
	if (!sch->seed)
		return -ENOMEM;

	dev_info(sch->ss->dev, "%s %d\n", __func__, sch->seedsize);
			       ...
			       size_t max, bool wait)
{
	...
	/* SS need a buffer to get next IV */
	if (ss->variant->is_ss) {
		ss->chanlist[flow].next_iv = kmalloc(sch->seedsize, GFP_KERNEL);
	...
	ss->chanlist[flow].method = sch->ce_op;
...
fail:
	...
	if (!ret) {
		memcpy(buf, data, len);
		/* Uglyness: BSP driver do not copy a next iv
		 * But getting next IV from CNT register give only 16bytes
		 * so last 6bytes of IV is never changed
		 */
		if (ss->chanlist[flow].next_iv)
			memcpy(sch->seed, ss->chanlist[flow].next_iv, sch->seedsize);
/*
		print_hex_dump(KERN_INFO, "RNG ", DUMP_PREFIX_NONE, 16, 1, data,
		       sch->datasize, false);
		print_hex_dump(KERN_INFO, "IV  ", DUMP_PREFIX_NONE, 16, 1, sch->seed,
		       sch->seedsize, false);
		*/
	}
	kzfree(data);
			    ...
			    u32 ce_op, struct sun8i_ss_ctx *ss)
{
	...
	dev_info(ss->dev, "Registered %s (op=%x)\n", name, h->ce_op);
