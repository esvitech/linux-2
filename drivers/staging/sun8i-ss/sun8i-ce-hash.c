...
int sun8i_ss_do_task(struct ahash_request *areq, int j)
{
	...
	ss->chanlist[flow].method = op->mode;
	ret = sun8i_ce_run_task(ss, flow, crypto_tfm_alg_name(areq->base.tfm));
	if (op->hash[0] != 0) {
...
hash_final2:
	...
	if (algt->ce_algo_id == CE_ID_HASH_MD5 ||
		algt->ce_algo_id == CE_ID_HASH_SHA1 ||
		algt->ce_algo_id == CE_ID_HASH_SHA224 ||
		algt->ce_algo_id == CE_ID_HASH_SHA256) {
	...
	if (algt->ce_algo_id == CE_ID_HASH_MD5) {
		...
		if (algt->ce_algo_id == CE_ID_HASH_SHA1 ||
		    algt->ce_algo_id == CE_ID_HASH_SHA224 ||
		    algt->ce_algo_id == CE_ID_HASH_SHA256) {
